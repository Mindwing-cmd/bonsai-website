...
interface CelestialSphereProps {
  ...
  zoom?: number;
  particleSize?: number;
  /** 0 = keine Mausreaktion, 1 = volle Reaktion (default 0.2) */
  mouseInfluence?: number;
  /** Sättigung 0–1, niedriger = mehr Grau/Steel (default 0.45 für BonS-AI) */
  saturation?: number;
  className?: string;
}
...
export const CelestialSphere: React.FC<CelestialSphereProps> = ({
  ...
  reduceMotionRef.current = usePrefersReducedMotion();
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Warp effect based on mouse (reduziert für dezente Reaktion)
        vec2 mouse_normalized = u_mouse / u_resolution;
        uv += (mouse_normalized - 0.5) * 0.8 * u_mouse_influence;
        ...
        // Final color: BonS-AI-nah (Electric Blue / Steel Graphite), geringere Sättigung
        float nebula = pow(t, 2.0);
        vec3 color = hsl2rgb(vec3(u_hue / 360.0 + nebula * 0.15, u_saturation, 0.42));
        color *= nebula * 2.2;
        // Leichter Grau-Mix für Steel-Graphite-Charakter
        vec3 steel = vec3(0.58, 0.62, 0.72);
        color = mix(steel * 0.15, color, 0.9);
    ...
    const init = () => {
      ...
      material = new THREE.ShaderMaterial({
        ...
        uniforms: {
          ...
          u_mouse_influence: { value: mouseInfluence },
  ...
  }, [hue, speed, zoom, particleSize, mouseInfluence, saturation]);
